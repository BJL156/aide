...
namespace brayjl {
	...
	void Shader::startup(const std::string vertexFilepath, const std::string fragmentFilepath) {
		createShaderProgram(vertexFilepath, fragmentFilepath);
	}
	...
	void Shader::setMat3(std::string uniformName, glm::mat3& mat3) {
		glUniformMatrix3fv(glGetUniformLocation(m_id, uniformName.c_str()), 1, GL_FALSE, glm::value_ptr(mat3));
	}

	void Shader::setMat2(std::string uniformName, glm::mat2& mat2) {
	...
	void Shader::setVec2(std::string uniformName, glm::vec2& vec2) {
		glUniform2fv(glGetUniformLocation(m_id, uniformName.c_str()), 1, glm::value_ptr(vec2));
	}

	void Shader::setInt(std::string uniformName, int& integer) {
		glUniform1i(glGetUniformLocation(m_id, uniformName.c_str()), integer);
	}

	void Shader::setFloat(std::string uniformName, float& flt) {
		glUniform1f(glGetUniformLocation(m_id, uniformName.c_str()), flt);
	}

	void Shader::setDouble(std::string uniformName, double& dble) {
	...
	void Shader::createShaderProgram(const std::string vertexFilepath, const std::string fragmentFilepath) {
		std::string vertexShaderCode = Utils::getFileContents(vertexFilepath);
		std::string fragmentShaderCode = Utils::getFileContents(fragmentFilepath);
